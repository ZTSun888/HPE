...
class _Optblock(nn.Module):
    def __init__(self, joint_num, bone_num):
        ...
        self.xy_conv = make_conv2d_layers([self.joint_num, self.joint_num, self.joint_num, self.joint_num], bnrelu_final=True)
        self.xy_1dconv = make_conv_layers([self.joint_num + 256, 128, self.joint_num],  bnrelu_final=True)
        self.mlp_head_xy_z = nn.Linear(3136, cfg.output_hm_shape[0])
        # self.mlp_head_xy_z = nn.Linear(4096, cfg.output_hm_shape[0])


    def forward(self, pred_2d, feature, pred_xy):
        ...
        pred_z2 = self.xy_conv(pred_xy)
        feature_cat = torch.cat((feature, pred_z2), dim=1)
        pred_z2 = rearrange(feature_cat, 'b c h w -> b c (h w)')
        pred_z2 = self.xy_1dconv(pred_z2)
        # print(pred_z2.shape)
        pred_z2 = self.mlp_head_xy_z(pred_z2)
...
class PoseNet(nn.Module):
    def __init__(self, joint_num, joint_bone_num):
        ...
        # self.mlp_head_x = nn.Linear(4096, cfg.output_hm_shape[0])
        # self.mlp_head_y = nn.Linear(4096, cfg.output_hm_shape[1])

        self.opt_block = _Optblock(self.joint_num, self.joint_bone_num)

        self.apply(self._init_weights)

        # self.freeze(self.deconv_layers)
        # self.freeze(self.final_layer)
        # self.freeze(self.mlp_head_x)
        # self.freeze(self.mlp_head_y)
    ...
    def forward(self, feature):
        ...
        pred_xy = torch.matmul(pred_x[:, :, :, None].detach(), pred_y[:, :, None, :].detach())

        # pred_z, bone_dir = self.opt_block(pred_2d.detach(), feature, pred_xy)
        pred_z = self.opt_block(pred_2d.detach(), feature_vector, pred_xy)
