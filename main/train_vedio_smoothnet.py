...
from common.base import TrainerVedio_Smoothnet
...
def main():
    ...
    trainer = TrainerVedio_Smoothnet()
    trainer._make_model()
    ...
    for epoch in range(trainer.start_epoch, cfg.end_epoch):
        ...
        running_loss['smooth'] = 0

        for i, (inputs, meta_info, targets) in enumerate(trainer.batch_generator):
            trainer.read_timer.toc()
            trainer.gpu_timer.tic()

            trainer.optimizer.zero_grad()

            loss = trainer.model(inputs, meta_info, targets, 'train')
