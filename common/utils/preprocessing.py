...
def load_bonelens(path, joint_num):
    # warning: two wrists has no bone length
    bonelens = np.zeros((joint_num, 2), dtype=np.float32)
    with open(path) as fp:
        i = 0
        for line in fp:
            if line[0] == '#': continue
            splitted = line.split(' ')
            joint_name, joint_id, joint_parent_id, bone_mean, bone_sigma= splitted
            joint_id, joint_parent_id, bone_mean, bone_sigma = int(joint_id), int(joint_parent_id), float(bone_mean), float(bone_sigma)
            # bonelens[joint_id]['name'] = joint_name
            # bonelens[joint_id]['parent_id'] = joint_parent_id
            # bonelens[joint_id]['mean'] = bone_mean
            # bonelens[joint_id]['3sigma'] = math.sqrt(bone_sigma) * 3
            bonelens[joint_id, 0] = bone_mean - (math.sqrt(bone_sigma) * 3)
            bonelens[joint_id, 1] = bone_mean + (math.sqrt(bone_sigma) * 3)
            i += 1

    return bonelens


def load_joint2d(path, joint_num):
    joint_type = {'right': np.arange(0, joint_num), 'left': np.arange(joint_num, joint_num * 2)}
    root_joint_idx = {'right': 20, 'left': 41}
    joint_2d = np.zeros((joint_num * 2, 3), dtype=np.float32)
    with open(path) as fp:
        i = 0
        for line in fp:
            splitted = line.split(' ')
            # print(splitted)
            joint_2d[i,0] = float(splitted[2])
            joint_2d[i,1] = float(splitted[4])
            joint_2d[i,2] = float(splitted[5])
            # print(splitted[2] + ' + ' + splitted[4] + ' + ' + splitted[5])
            i += 1
    joint_2d[:, 0] = joint_2d[:, 0] / cfg.input_img_shape[1] * cfg.output_hm_shape[2]
    joint_2d[:, 1] = joint_2d[:, 1] / cfg.input_img_shape[0] * cfg.output_hm_shape[1]
    joint_2d[joint_type['right'], 2] = joint_2d[joint_type['right'], 2] - joint_2d[root_joint_idx['right'], 2]
    joint_2d[joint_type['left'], 2] = joint_2d[joint_type['left'], 2] - joint_2d[root_joint_idx['left'], 2]

    joint_2d[:, 2] = (joint_2d[:, 2] / (cfg.bbox_3d_size / 2) + 1) / 2. * cfg.output_hm_shape[0]
    return joint_2d
...
def transform_input_to_output_space(joint_coord, joint_valid, rel_root_depth,  root_joint_idx,
                                        joint_type):
