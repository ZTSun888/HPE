...
class DatasetVedio_Smoothnet(torch.utils.data.Dataset):
    ...
    def __getitem__(self, idx):
        ...
        gt_data = self.datalist[vedio_idx]['datalist'][img_idx]
        ...
        gt_joint_coord_list = []
        video_len = len(self.datalist[vedio_idx]['datalist'])

        # for i in range(img_idx - int(self.pad/2) + 1, img_idx + int(self.pad/2) + 1):
        for i in range(img_idx - self.pad + 1, img_idx + 1):
            if i < 0:
                data = self.datalist[vedio_idx]['datalist'][0]
            elif i >= video_len:
                data = self.datalist[vedio_idx]['datalist'][video_len-1]
            else:
        ...
        gt_joint_coord_list[:, :, 0] = (gt_joint_coord_list[:, :, 0] - (img_width / 2)) / (img_width / 2)
        gt_joint_coord_list[:, :, 1] = (gt_joint_coord_list[:, :, 1] - (img_height / 2)) / (img_height / 2)
        gt_joint_coord_list[:, :, 2] = gt_joint_coord_list[:, :, 2] / 200
        ...
        return inputs,  meta_info, targets
    ...
    def cal_motion_loss(self, trail, pad):
        ...
        for i in range(0, len(trail) - time_interval):
            ...
            motion_loss_opt[i] = np.sum(abs(np.cross(trail[i], trail[i + time_interval])))
